import os
from django.conf import settings
from django.shortcuts import render

from django.shortcuts import render
from django.http import JsonResponse
import nltk
import json
import pickle
import numpy as np
import random
import tensorflow as tf
import re
import unicodedata

texts_path = os.path.join(settings.BASE_DIR, 'chatbot/static', 'texts.pkl')
labels_path = os.path.join(settings.BASE_DIR, 'chatbot/static', 'labels.pkl')
model_path = os.path.join(settings.BASE_DIR, 'chatbot/static', 'model.keras')
data_path = os.path.join(settings.BASE_DIR, 'chatbot/static', 'data.json')

ds_words = pickle.load(open(texts_path, 'rb'))
ds_nhan = pickle.load(open(labels_path, 'rb'))
model = tf.keras.models.load_model(model_path, compile=False)
data = json.loads(open(data_path, encoding='utf-8').read())

def xuly_dulieu(text):
    n = ''.join(c for c in unicodedata.normalize('NFD', text) if unicodedata.category(c) != 'Mn')
    dc = re.sub(r'[^\w\s]', ' ', n)
    xl_word = nltk.word_tokenize(dc)
    xl_word = [word.lower() for word in xl_word]
    return xl_word

def bow(input, words):
    xl_input = xuly_dulieu(input)
    bag = [0] * len(words)
    for s in xl_input:
        for i, w in enumerate(words):
            if w == s:
                bag[i] = 1
    return np.array(bag)

def dudoan_nhan(input, model):
    p = bow(input, ds_words)
    res = model.predict(np.array([p]))[0]
    Nguong = 0.25
    kq_dudoan = []
    for i, r in enumerate(res):
        if r > Nguong:
            kq_dudoan.append([i, r])
    kq_dudoan.sort(key=lambda x: x[1], reverse=True)
    return_list = []
    for r in kq_dudoan:
        return_list.append({"nhan": ds_nhan[r[0]], "xacsuat": str(r[1])})
    return return_list

def get_phanhoi(ints, data_json):
    tag = ints[0]['nhan']
    yd = data_json['intents']
    for i in yd:
        if i['tag'] == tag:
            phanhoi = random.choice(i['responses'])
            break
    return phanhoi


def home(request):
    return render(request, 'chatbot/chatbot.html')

def get_bot_response(request):
    userText = request.GET.get('msg')
    if userText:
        ints = dudoan_nhan(userText, model)
        res = get_phanhoi(ints, data)
        return JsonResponse({'response': res})
    else:
        return JsonResponse({'response': "Không có thông tin đầu vào"})

